    ...
    if srt_path:
        ...
        try:
            ...
            burn_subtitles_with_moviepy(
                input_video, output_path, srt_path, start, duration, subtitle_config
            )
        except Exception as e:
...
def _merge_single_collection(
    collection: dict, index: int, clips_dir: Path, output_dir: Path
) -> None:
    """合并单条合集。"""
    ...
    try:
        ...
        subprocess.run(
            cmd, check=True, capture_output=True, timeout=MERGE_TIMEOUT_SECONDS
        )
        collection["video_path"] = str(
    ...
    try:
        ...
        subprocess.run(
            [
                ...
                "1",
                "-q:v",
                str(CLIP_THUMB_QUALITY),
                "-vf",
                f"scale={CLIP_THUMB_SCALE}:-1",
                str(clip_thumb),
            ],
            check=True,
            capture_output=True,
...
def _generate_project_thumbnail(
    project_id: str, clips: list[dict], output_dir: Path
) -> None:
    ...
    try:
        ...
        subprocess.run(
            [
                ...
                "1",
                "-q:v",
                str(COVER_THUMB_QUALITY),
                "-vf",
                f"scale={COVER_THUMB_SCALE}:-1",
                str(thumb_path),
            ],
            check=True,
            capture_output=True,
